...
def k_nearest_neighboor(data, predict, k=3):
	...
	confianca=Counter(votos).most_common(1)[0][1] / k
	return votos_resultado, confianca
...
for i in range(5):
	...
	for group in test_set:
		for data in test_set[group]:
			voto,confianca= k_nearest_neighboor(train_set, data, k=5)
			if group == voto:
				correct+=1
			#else:
				#print(confianca)
	...
	precisoes.append(correct/total)

print(sum(precisoes)/len(precisoes))